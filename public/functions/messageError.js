...
async function errorAPI(discord, _code, message) {
  const code = Number(_code);

  if (arguments.length !== 3) { throw new Error('[1] error estatico utility/ErrorAPI') }
  if (!(discord instanceof Message || discord instanceof BaseInteraction)) { throw new Error('[2] error estatico argumentos utility/send') }
